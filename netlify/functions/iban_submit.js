...
function ibanIsValid(raw){
  ...
  let rem=0,buf=''; const toNum=(ch)=>{const c=ch.charCodeAt(0); if(c>=48&&c<=57) return ch; if(c>=65&&c<=90) return String(c-55); return '';};
...
exports.handler = async (event) => {
  try {
    ...
    // Update: nur vereinbarte IBAN-Properties + Audit + Status
    const update = [
      { Name:'iban',               Value: iban },
      { Name:'ip_iban',            Value: ip },
      { Name:'agent_iban',         Value: agent },
