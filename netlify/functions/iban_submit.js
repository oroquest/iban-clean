...
for(const ch of rearr){buf+=toNum(ch); while(buf.length>=7){rem=Number(String(rem)+buf.slice(0,7))%97; buf=buf.slice(7);} }
if(buf.length) rem=Number(String(rem)+buf)%97; return rem===1;}
...
exports.handler = async (event)=>{
  try{
    ...
    const cJson=await cRes.json(); if(!cRes.ok||!cJson?.Data?.length) return { statusCode:404, body:'contact_not_found' };
    const contactId=cJson.Data[0].ID;

    const cdRes=await fetch(`${mjBase}/v3/REST/contactdata/${contactId}`,{headers:{Authorization:mjAuth}});
    const cdJson=await cdRes.json(); const props={}; if(cdJson?.Data?.[0]?.Data) for(const kv of cdJson.Data[0].Data) props[kv.Name]=kv.Value;

    if(props.token_iban!==token) return { statusCode:403, body:'Invalid token' };
    const expiryTs=new Date(props.token_iban_expiry||0).getTime(); const testMode=String(process.env.IBAN_TEST_MODE||'0')==='1';
    ...
    const uRes=await fetch(`${mjBase}/v3/REST/contactdata/${contactId}`,{method:'PUT',headers:{Authorization:mjAuth,'Content-Type':'application/json'},body:JSON.stringify({Data})});
